import os # Executes shell commands (such as pip)
...
def check_and_install_dependencies_not_kivymd():
    for i in range (len(dependencies)):
        try: # Tries to import it
            ...
            os.system(f"{sys.executable} -m pip install kivy==2.3.1")
            ...
            if dependencies[i] == "kivy":
                ...
                os.system(f"{sys.executable} -m pip install {dependencies[i]}")
...
def check_and_install_kivymd():
    try: # Tries to import it
        ...
        os.system(f"{sys.executable} -m pip install https://github.com/kivymd/KivyMD/archive/master.zip")
